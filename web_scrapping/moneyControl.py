...
html= urllib.request.urlopen(base_url).read()# step 1: get html
soup=BeautifulSoup(html,'html.parser') #step 2:get a parse tree
tags=soup('th')
col_name=['sl_no','company_name']
for tag in tags :
    c=tag.get('data-tooltip',None) #step 3 gets the specified tag
    if c is not None :
